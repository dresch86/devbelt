...
COPY ./leantime/start.sh /start.sh
RUN chmod +x /start.sh

COPY ./leantime/config/custom.ini /usr/local/etc/php/conf.d/custom.ini

# Configure supervisord
COPY ./leantime/config/supervisord.conf /etc/supervisor/conf.d/supervisord.conf
COPY ./leantime/config/app.conf  /etc/apache2/conf.d/app.conf
